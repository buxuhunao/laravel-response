...
namespace Three\LaravelResponse\Support;
...
class Format implements ResponseFormat
{
    ...
    public function paginator(AbstractPaginator|AbstractCursorPaginator|Paginator $resource): array
    {
        return [
            ...
            ...$this->metaWrap($this->formatMeta($resource)),
        ];
    }
    ...
    public function resourceCollection(ResourceCollection $collection): array
    {
        return [
            ...
            ...$this->metaWrap(array_merge_recursive($this->formatMeta($collection->resource), $collection->with(request()), $collection->additional)),
        ];
    }

    protected function metaWrap($data)
    {
        if ($this->config['meta']['show'] ?? false) {
            return [$this->config['meta']['alias'] ?? 'meta' => $data];
        }
